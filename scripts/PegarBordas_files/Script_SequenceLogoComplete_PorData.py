...
parser.add_argument('Period', help=' inicial, final. Format:  "0000-00,0000-00"')
...
dateList = pd.date_range(period[0], period[1], freq='MS').strftime("%Y-%m").tolist()
for n in range(0, len(dateList)):
    ...
    patternString = '.+_EPI_ISL_\d+_' + str(date) + '.+.\.bordas\.' + str(args.NumBorder) + '_\d+_\d+\.' + args.SeqType + '\.fa$'
    ...
    for file in bordasFiles:
        ...
        codons = []
        ntAmount = 3
        SeqStorage_Name = date + args.SeqStorage
        with open(SeqStorage_Name, 'w') as SeqStorage:
        ...
        MatraixProb_name = 'PROB_' + args.Matrix + date
        MatraixDF.to_csv(sep="\t", header=True, path_or_buf=MatraixProb_name, index=True)
        print(f'Matrix: \n{MatraixDF}\n')


        if args.SeqType.upper().strip() == 'NT':
            matrixSymbol_name = 'PROB_' + args.Matrix + '_Symbol' + date
        ...
        matrixBit_name = "BIT_" + args.Matrix + date
        ...
        if args.SeqType.upper().strip() == 'AA':
            ...
            figsave_name = date + '_' + args.SeqLogo + '.png'
            ...
            figsave_name = date + '_' + args.SeqLogo + '.png'
            fig.savefig(figsave_name, transparent=True)
